...
*/
	//a�ade una funcion a la clase String.
...
var Util={
		imprimirBoleto:function()
		{
			if (window.print)
			{
                ...
                /*consultarChofer:function()
		{
                    formu.action="consultar_chofer.php?";
                    if(confirm("Desea realizar la consulta de choferes?"))
			{
		...
		realizarCompra:function()
		{
			...
			var len=document.forms[0].length; //es 1
		...
		avanzar:function()
		{
			...
			if(bolSel==0)
			{
					alert("Tienes que  seleccionar m�s boletos");
					return false;
			}
			if(bolSel>bolMax )
			{
				if(confirm("Tienes Seleccionados m�s boletos, deseas volver a seleccionarlos!?"))
				{
					location.reload(true);
				}else
				{
					alert("Puedes seleccionar m�s boletos, cambiando el numero de boletos.");
